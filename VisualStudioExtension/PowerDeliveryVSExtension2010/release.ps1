...
function BuildSource {
	...
	msbuild PowerDeliveryVSExtension.2010.csproj "/consoleloggerparameters:Verbosity=q" /p:Configuration=Release /nologo
...
function UpdateNuspec {
	...
	$script:nuspecFullPath = Join-Path $currentDirectory .\PowerDelivery-VSExtension-2010.nuspec
...
function PackageChocolatey {
	...
	"$nuspecFullPath -> PowerDelivery-VSExtension-2010.$($newVersion).nupkg"
...
"----------------------------------------------------------"
"Releasing new version of powerdelivery-vsextension-2010..."
"----------------------------------------------------------"

del *.nupkg

$latestVersionMessage = $(clist powerdelivery-vsextension-2010)

$script:currentDirectory = Get-Location

if ($latestVersionMessage.Trim() -eq "No packages found.") {
    $latestVersion = "1.0.0.0"
    $script:newVersion = "1.0.0.0"
}
else {
